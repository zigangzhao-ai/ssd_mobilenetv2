...
import os
import mobilenetv2_cc as mobilenetv2
from attention.attention import DWConv2d
...
def conv_dw_bn(inp, oup, stride):
    return nn.Sequential(
        DWConv2d(inp, oup, 3, stride, 1, bias=False),
        nn.BatchNorm2d(oup),
...
def add_extras(i):
    ...
    layers += [conv_dw_bn(256,512,2)]

    #conv15
    layers += [conv1_bn(512,128,1)]
    # layers += [conv_bn(128,256,2)]
    layers += [conv_dw_bn(128,256,2)]
...
def multibox(mobilenet, extra_layers, cfg, num_classes):
    ...
    loc_layers += [DWConv2d(96, 4 * 4, kernel_size=1)]
    conf_layers += [DWConv2d(96, 4 * num_classes, kernel_size=1)]

    loc_layers += [DWConv2d(1280, 6 * 4, kernel_size=1)]
    conf_layers += [DWConv2d(1280, 6 * num_classes, kernel_size=1)]

    loc_layers += [DWConv2d(512, 6 * 4, kernel_size=1)]
    conf_layers += [DWConv2d(512, 6 * num_classes, kernel_size=1)]

    loc_layers += [DWConv2d(256, 6 * 4, kernel_size=1)]
    conf_layers += [DWConv2d(256, 6 * num_classes, kernel_size=1)]

    loc_layers += [DWConv2d(256, 6 * 4, kernel_size=1)]
    conf_layers += [DWConv2d(256, 6 * num_classes, kernel_size=1)]

    loc_layers += [DWConv2d(128, 6 * 4, kernel_size=1)]
    conf_layers += [DWConv2d(128, 6 * num_classes, kernel_size=1)]

    # for k, v in enumerate(extra_layers[1::2], 2):
    # for k, v in enumerate(extras_source):
    #     k += 2
    #     loc_layers += [DWConv2d(extra_layers[v][0].out_channels,
    #                              cfg[k] * 4, kernel_size=1)]
    #     conf_layers += [DWConv2d(extra_layers[v][0].out_channels,
