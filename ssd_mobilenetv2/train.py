...
# from ssd_fpn_concat import build_ssd
# from ssd_nolocal import build_ssd
# from ssd_focalloss import build_ssd
from ssd_cc import build_ssd
...
model_save_dir = "weights/ssd_mobilenetv2"
...
parser.add_argument('--basenet', default="mobilenetv2.pth.tar", #"mb2-imagenet-71_8.pth",
...
def train():
    if args.dataset == 'COCO':
        ...
        dataset = VOCDetection(root=args.dataset_root,
                               ...
                               #transform_mosaic=SSDAugmentation_mosaic(cfg['min_dim'],MEANS),
                               #use_mosaic=True)
    ...
    if args.resume:
        ...
        # ssd_net.mobilenet.apply(weights_init)  --> have problem
    ...
    if not args.resume:
        ...
        # ssd_net.loc.apply(weights_init)
        # ssd_net.conf.apply(weight_init_focalloss)
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        ...
        if iteration >= 5000 and iteration % 500 == 0:
            ...
            torch.save(ssd_net.state_dict(), model_save_dir + '/'+ 'mobilenetv2_' +
                       repr(iteration) + '.pth')
    torch.save(ssd_net.state_dict(), model_save_dir + '/' + 'mobilenetv2_final' + '.pth')
...
def weight_init_focalloss(m):
    if isinstance(m, nn.Conv2d):       
        prior = 0.01
        bias_value = -math.log((1.0 - prior) / prior)
        xavier(m.weight.data)
        torch.nn.init.constant_(m.bias.data, bias_value)
