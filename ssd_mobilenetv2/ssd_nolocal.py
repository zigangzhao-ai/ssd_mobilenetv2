...
from attention.attention import Bottleneck, SEModule, ECAModule, NonLocalBlockND
from data.config import USE_Nonlocal
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        if phase == 'test':
            ...
            self.detect = Detect()
            # SE模块【6个特征层：224 672 512 512 256 128】
        if USE_Nonlocal:
            print("use nonlocal")
            self.Nonlocal1 = NonLocalBlockND(32)
            self.Nonlocal2 = NonLocalBlockND(96)
            #self.Nonlocal2 = NonLocalBlockND(1280)
            #self.Nonlocal3 = NonLocalBlockND(512)
            #self.Nonlocal4 = NonLocalBlockND(256)
            #self.SE5 = Bottleneck(256)
            #self.SE6 = Bottleneck(128)
    ...
    def forward(self, x):
        ...
        conf = list()
        # attention = list()
        # attention.append(self.Nonlocal1)
        # attention.append(self.Nonlocal2)
        # attention.append(self.Nonlocal3)
        # attention.append(self.Nonlocal4)
        ...
        if NonlocalBLOCK == 5:
            ...
            x = self.mobilenet.activation(x)  ##[150*150]
            #print(x.shape)
        ...
        if NonlocalBLOCK == 1:
            ...
            for i in self.mobilenet.bottlenecks[:5]:
                x = i(x)
                #print(x.shape)
            # x = self.mobilenet.bottlenecks[4](x)
            #s = self.L2Norm(x)
            #print(x.shape)
            x = self.Nonlocal1(x)
            sources.append(x)
        ...
        # for i, x in enumerate(sources[:4]):
        #     # print(i, x.shape)
        #     x = attention[i](x)
        #     # print(type(x))
