...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        #self.conv_256_512 = DWConv2d(in_channels=256, out_channels=512, kernel_size=1, stride=1)
        self.conv_256_512 = nn.Conv2d(in_channels=256, out_channels=512, kernel_size=1, stride=1)
        #layer19_1到layer19_1
        self.conv_512_512 = nn.Conv2d(in_channels=512, out_channels=512, kernel_size=1, stride=1)

        ##add后512-->1280 layer19_1到layer19 上采样，尺度大一倍
        self.upsample_512_512 = Upsample(10)
        #self.conv_512_1280 = DWConv2d(in_channels=512, out_channels=1280, kernel_size=1, stride=1)
        self.conv_512_1280 = nn.Conv2d(in_channels=512, out_channels=1280, kernel_size=1, stride=1)
        # layer19到layer19  尺度不变
        self.conv_1280_1280 = nn.Conv2d(in_channels=1280, out_channels=1280, kernel_size=1, stride=1)
        
        # layer19 到 layer15  上采样，尺度大一倍
        self.upsample_1280_1280 = Upsample(19)
        #self.conv_1280_96 = DWConv2d(in_channels=1280, out_channels=96, kernel_size=1, stride=1)
        self.conv_1280_96 = nn.Conv2d(in_channels=1280, out_channels=96, kernel_size=1, stride=1)
        ...
        self.smooth1 = nn.Conv2d(1280, 1280, kernel_size=3, padding=1, stride=1)
        self.smooth2 = nn.Conv2d(96, 96, kernel_size=3, padding=1, stride=1)
        ...
        if USE_SE:
            self.SE1 = SEModule(96)
            self.SE2 = SEModule(1280)
    ...
    def forward(self, x):
        ...
        # layer19_1层融合结果  self.bn1(self.conv1(x)) 在通道维度上融合
        conv19_11 = self.bn1(self.conv_256_512(self.upsample_256_256(sources[3]))) + self.bn1(self.conv_512_512(sources[2]))
        conv19_1 = self.smooth(conv19_11)
  
        # layer19层融合结果
        layer19_m =  self.bn2(self.conv_512_1280(self.upsample_512_512(conv19_11))) + self.bn2(self.conv_1280_1280(sources[1]))
        layer19 = self.smooth1(layer19_m)
     
        # layer15层融合结果
        conv15 = self.bn3(self.conv_1280_96(self.upsample_1280_1280(layer19_m))) + self.bn3(self.conv_96_96(sources[0]))      
        conv15 = self.smooth2(conv15)
        ...
        if USE_SE:
            sources_final.append(self.SE1(conv15))
            sources_final.append(self.SE2(layer19))
            sources_final.append(self.SE3(conv19_1))
            ...
            #print("no cbam")
            sources_final.append(conv15)
            sources_final.append(layer19)
            sources_final.append(conv19_1)
